...
for t_path in TEXT_PATHS:
    ...
    for root, dirs, files in os.walk(t_path):
        ...
        # PRIORITÄT: Wir wollen NUR 'eng' (English). 
        # Wir ignorieren alles, was in einem Sprach-Ordner liegt, der NICHT 'eng' ist.
        # Aber wir müssen vorsichtig sein: manche Mods haben configs/text/st_...xml direkt.
        ...
        # Wenn 'text' im Pfad ist, aber kein 'eng', prüfen wir ob es ein Mod-Ordner ohne Unterordner ist
        # In GAMMA ist die Struktur meist: .../configs/text/eng/file.xml
        if 'text' in rel_parts and 'eng' not in rel_parts:
            # Check ob ein anderer Sprachordner auf der gleichen Ebene wie 'eng' existiert
            # Wenn ja, und wir sind nicht in 'eng', dann skippen wir.
...
# Radikale Liste von Scope-Suffixen und Müll
JUNK_SUFFIXES = [
                ...
                try:
                    with open(os.path.join(root, f), 'r', encoding='utf-8', errors='ignore') as l:
                        ...
                        for line in l:
                            ...
                            # Stalker LTX Syntax: [section]:parent oder ![section] oder [section]
                            if line.startswith('['):
                                # Extrahiere den Teil in den Klammern [...]
                                ...
                                # Suche nach einem Parent nach der schließenden Klammer
                                remainder = line[end_bracket_idx+1:].strip()
                                if remainder.startswith(':'):
                                    parent = remainder[1:].strip().split(',')[0].strip() # Nur erste Parent-Basis
                                
                                # FILTER: Wenn Sektion ein Scope-Suffix hat -> Skip
...
final = []
# Zuerst alle "echten" Waffen identifizieren (die z.B. rpm, hit_power etc haben oder von einer Basis erben)
for sec, d in tqdm.tqdm(registry.items()):
    if not sec.startswith("wpn_") or "_hud" in sec: continue
    
    # DLTX: Scopes etc. ignorieren, aber NIMBLE ist eine echte Waffe
...
# --- ICON EXTRAKTION ---
print("🖼️ Suche Texturen für Icons...")
import subprocess
...
print(f"✂️ Extrahiere {len(df_final)} Icons...")
converted_cache = {}
...
for _, r in tqdm.tqdm(df_final.iterrows(), total=len(df_final)):
    ...
    if tex_key in converted_cache:
        ...
        # STALKER Inventory Grid nutzt 50px pro Zelle für inv_grid_* Koordinaten.
        cell_size = 50
...
print(f"✅ Fertig! {len(df_final)} Waffen verarbeitet.")
